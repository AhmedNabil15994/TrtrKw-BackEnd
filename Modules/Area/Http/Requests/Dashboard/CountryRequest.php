        ...
        switch ($this->getMethod()) {
            ...
            case 'POST':
                ...
                return [
                    ...
                    'title.*' => ['required', Rule::unique('country_translations', 'title')->where(function ($query) {
                        return $query->where('country_id', Country::whereNull('deleted_at')->where('code', $this->code)->value('id'));
                    })],
                ];
            ...
            case 'PUT':
                return [
                    ...
                    'title.*' => ['required', Rule::unique('country_translations', 'title')->ignore($this->id, 'country_id')->where(function ($query) {
                        return $query->where('country_id', Country::whereNull('deleted_at')->where('id', $this->id)->value('id'));
                    })],
                ];
        }
    }
