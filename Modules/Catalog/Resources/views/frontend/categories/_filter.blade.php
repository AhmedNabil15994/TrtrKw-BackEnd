<div class="content-sidebar">
    ...
    <div class="panel-group filter-options" id="accordionNo">
        ...
        <form method="get"
              action="{{ route('frontend.categories.products', $category ? $category->translate(locale())->slug : null) }}">
                        ...
                        <div class="checkboxes radios one-in-row">
                            ...
                            @foreach($tags as $k => $tag)
                                <input id="check-{{ $tag->id }}"
                                       ...
                                       name="tags"
                                    {{ request()->get('tags') == $tag->translate(locale())->slug ? 'checked': '' }}>
                                <label for="check-{{ $tag->id }}"> {{ $tag->translate(locale())->title }}</label>
                            @endforeach
                        ...
                        <div class="checkboxes one-in-row">
                            ...
                            @foreach($categories as $k => $cat)
                                <input id="check-category-{{ $cat->id }}"
                                       value="{{ $cat->translate(locale())->slug }}"
                                       type="checkbox"
                                       name="categories[{{ $cat->id }}]"
                                    {{ (isset(request()->get('categories')[$cat->id]) && request()->get('categories')[$cat->id] == $cat->translate(locale())->slug) || ($category && $category->id == $cat->id) ? 'checked': '' }}>
                                <label
                                    for="check-category-{{ $cat->id }}"> {{ $cat->translate(locale())->title }}</label>
                            @endforeach
