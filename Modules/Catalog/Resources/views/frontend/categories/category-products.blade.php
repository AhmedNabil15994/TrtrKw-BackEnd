...
@section('content')
    ...
    {{--<div class="category-header d-flex align-items-center">
        <div class="container">
            @if($category)
                <p style="color: #FFFFFF; font-size: 16px;">{{ $category->translate(locale())->title }}</p>
                <img src="{{ url($category->image) }}" alt="{{ $category->translate(locale())->title }}"/>
            @endif
        </div>
    </div>--}}
                                ...
                                <div class="col-md-4 col-6">
                                    <div class="product-grid">
                                                ...
                                                @if($product->tags)
                                                    @foreach($product->tags as $i => $tag)
                                                        <label class="label-tag-product"
                                                               style="background-color: {{ $tag->background ?? '#ddd' }}; color: {{ $tag->color ?? '#fff' }}">{{ $tag->translate(locale())->title }}</label>
                                                    @endforeach
                                                @endif
                                                    ...
                                                    <li>
                                                        ...
                                                        <form class="general-form" method="POST">
                                                            ...
                                                            <a href="javascript:;"
                                                               ...
                                                               onclick="generalAddToCart('{{ route("frontend.shopping-cart.create-or-update", [ $product->translate(locale())->slug ]) }}', '{{ $product->id }}')"
                                        ...
                                        <div class="product-content">
                                            <h3 class="title">
                                                <a href="{{ route('frontend.products.index', $product->translate(locale())->slug) }}">{{ $product->translate(locale())->title }}</a>
                                            </h3>
