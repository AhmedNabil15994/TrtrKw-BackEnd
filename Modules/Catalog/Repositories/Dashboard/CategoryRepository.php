...
class CategoryRepository
{
    ...
    public function create($request)
    {
        ...
        try {
            ...
            if (!is_null($request->cover)) {
                $imgName = $this->uploadImage(public_path(config('core.config.category_img_path')), $request->cover);
                $data['cover'] = config('core.config.category_img_path') . '/' . $imgName;
            } else {
    ...
    {
        $query = $this->category->with(['translations'])
