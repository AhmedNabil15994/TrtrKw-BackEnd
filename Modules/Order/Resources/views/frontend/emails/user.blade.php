@component('mail::message')
    ...
    @component('mail::table')
        ...
        @foreach ($order->orderProducts->mergeRecursive($order->orderVariations) as $orderProduct)
            {{ isset($orderProduct->product_variant_id) && !empty($orderProduct->product_variant_id) ? generateVariantProductData($orderProduct->variant->product, $orderProduct->product_variant_id, $orderProduct->variant->productValues->pluck('option_value_id')->toArray())['name'] : $orderProduct->product->translate(locale())->title }} | {{ $orderProduct->sale_price }} KWD | {{ $orderProduct->qty }} | {{$orderProduct->total}} KWD
        @endforeach
